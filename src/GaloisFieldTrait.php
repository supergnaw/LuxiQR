...
//        }
    ...
    protected function galoisFieldMultiply(int $a, int $b): int
    {
        ...
        $log = $this->logTable[$a] + $this->logTable[$b];

        return $this->expTable[$this->gfWrap($log)];
    }
    ...
    protected function galoisFieldMultiplyManual(int $a, int $b): int
    {
        ...
        for ($i = 0; $i < 8; $i++) {
            // If the lowest bit of b is 1, XOR a into result
            if ($b & 1) $p ^= $a;

            $b >>= 1; // Shift b to the right by 1

            // Check if a's highest bit (bit 7) is set before shifting
            $carry = ($a & 0x80) ? 1 : 0;
     ...
     * @return int
     */
    protected function galoisFieldDivide(int $a, int $b): int
    {
        if ($b == 0) throw new LuxiQRException("Division by zero causes the universe to implode");

        if ($a == 0) return 0; // 0 divided by anything is still 0

        // G256 division is multiplication by the inverse
        return $this->galoisFieldMultiply($a, $this->galoisFieldInverse($b));
    }
     ...
     * @return int
     */
    protected function galoisFieldDivideManual(int $a, int $b): int
    {
        if ($b == 0) throw new LuxiQRException("Division by zero causes the universe to implode");

        if ($a == 0) return 0; // 0 divided by anything is still 0

        // Multiply a by the multiplicative inverse of b (since division is multiplication by the inverse in GF)
        return $this->galoisFieldMultiplyManual($a, $this->galoisFieldInverse($b));
    }
...
//            echo "<h3>Processing coefficient: {$dividend[$i]}</h3>";

            foreach ($divisor as $j => $coefficient) {
                $product = $this->galoisFieldMultiply($dividend[$i], $coefficient);
//                var_dump("Multiplying $dividend[$i] * $coefficient = $product");
    ...
    protected function getGeneratorPolynomial($degree): array
    {
        $lastPoly = [1];
        for ($d = 0; $d < $degree; $d++) {
            $lastPoly = $this->multiplyPolynomials($lastPoly, [1, $this->expTable[$this->gfWrap($d)]]);
        }
        return $lastPoly;
    }

    protected function createGeneratorPolynomial(int $degree): array
    {
        $generator = [1]; // Start with g(x) = 1

        for ($d = 0; $d < $degree; $d++) {
            // Create the term (x - α^i)
            $term = [1, $this->expTable[$d]]; // [1, α^$d]
    ...
    protected function gfWrap(int $i): int
    {
        return intval(($i % 256) + floor($i / 256));
    }
}