...
use supergnaw\LuxiQR\traits\DebugTrait;
...
class LuxiQRTest extends LuxiQR
{
    ...
    public function testDetectEncodingMode(): void
    {
        ...
        $tests = [
            [
                "data" => "8675309",
                "expected" => Modes::NUMERIC
            ],
            [
                "data" => "HELLO WORLD",
                "expected" => Modes::ALPHANUMERIC
            ],
            [
                "data" => "茗荷",
                "expected" => Modes::KANJI
            ],
            [
                "data" => "Hello, world!",
                "expected" => Modes::BYTE
        ...
        $values = [
            Modes::NUMERIC => "numeric",
            Modes::ALPHANUMERIC => "alphanumeric",
            Modes::KANJI => "kanji",
            Modes::BYTE => "byte"
        ];
        ...
        foreach ($tests as $test) {
            ...
            $table .= sprintf(
                ...
                str_pad(
                    ...
                    pad_string: GaloisFieldTables::DATA,
    ...
    public function testLookupTables(): void
    {
        ...
        for ($i = 0; $i < 256; $i += 16) {
            ...
            for ($j = 0; $j < 16; $j++) {
                $k = $i + $j;
                $status = ($expExpected[$k] == GaloisFieldTables::EXP[$k]) ? "passed" : "failed";

                if ("failed" == $status) $overallPassed = false;

                $expTable .= "<td><span class='$status'>{$expExpected[$k]}</span> / " . GaloisFieldTables::EXP[$k] . "</td>";
            }
            $expTable .= "</tr>";
        }
        $expTable .= "</table>\n";
        ...
        for ($i = 0; $i < 256; $i += 16) {
            ...
            for ($j = 0; $j < 16; $j++) {
                $k = $i + $j;
                $status = ($logExpected[$k] == GaloisFieldTables::LOG[$k]) ? "passed" : "failed";

                if ("failed" == $status) $overallPassed = false;

                $logTable .= "<td><span class='$status'>{$logExpected[$k]}</span> / " . GaloisFieldTables::LOG[$k] . "</td>";
            }
            $logTable .= "</tr>";
        }
        $logTable .= "</table>\n";
    ...
    public function testMultiply(): void
    {
        ...
        foreach ($tests as $test) {
            ...
            $table .= sprintf(
                ...
                $test["a"],
                $test["b"],
                GaloisFieldTables::LOG[$test["a"]],
                GaloisFieldTables::LOG[$test["b"]],
                GaloisFieldTables::LOG[$test["a"]] + GaloisFieldTables::LOG[$test["b"]],
                $expected,
                $result);
        }
        $table .= "</table>\n";
